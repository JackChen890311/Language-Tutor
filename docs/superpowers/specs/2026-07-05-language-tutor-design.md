...
## Fix: misspelled WORD_SUGGESTION marker leaked into displayed text; add furigana for all kanji

**Bug:** a real production example surfaced text like `コーラ<!--WORD_SUGGETION:{"word": "コーラ", "reading": "こーら"}--> (こーら)` rendering literally in the Lesson dialogue. The model is instructed (in both `chat_system_prompt` and `lesson_system_prompt`) to emit `<!--WORD_SUGGESTION:{...}-->`, but occasionally misspells it in its actual output (`WORD_SUGGETION`, missing the "S" before "TION"). Both places in `services/chat_service.py` that recognize the marker did an exact string match — `_WORD_SUGGESTION_RE = re.compile(r"<!--WORD_SUGGESTION:(.*?)-->")` for post-generation cleanup (`extract_word_suggestions`), and a literal `"WORD_SUGGESTION:" not in chunk` check inside `StreamCollector.__iter__` for live-streaming display — so a one-character typo caused the marker to be left in the visible text in *both* the streamed preview and the final saved message, instead of being stripped into a word suggestion.

**Fix:** loosened `_WORD_SUGGESTION_RE` to `r"<!--WORD_SUGGE\w*:(.*?)-->"` — matches any marker starting with the `WORD_SUGGE` prefix regardless of how "SUGGESTION" is spelled after that, with no realistic risk of false-positiving on real conversation text (the `<!--WORD_SUGGE...-->` shape is specific enough). `StreamCollector.__iter__`'s ad-hoc substring check is replaced with `_WORD_SUGGESTION_RE.search(chunk)`, so both code paths now share one source of truth for what counts as a word-suggestion marker.

**Furigana for all kanji:** the WORD_SUGGESTION mechanism only gives a reading for words the model decides are "newly introduced" — it was never meant to guarantee every kanji in a sentence is readable. Added `PromptBuilder._furigana_rule(target_lang)`, mirroring the existing `_chinese_rule(native_lang)` pattern: when `target_lang == "ja"`, both `chat_system_prompt` and `lesson_system_prompt` gain the instruction "whenever you write a word containing kanji (漢字) in Japanese text, immediately follow it with its reading in hiragana in parentheses ... for every kanji word, not just newly introduced vocabulary." No-op for every other target language.

## Fix: malformed `<speak>` opening tag also leaked into displayed text

Same failure class as the WORD_SUGGESTION marker above, reported immediately after: the model produced `<s speak>ペットボトルのコーラは冷たいです。</speak>` — an extra `s ` inside the opening tag — and `ui/components/audio_controls.py`'s `extract_speak_text`/`parse_message_segments` both matched on the exact literal `<speak>...</speak>`, so the malformed pair was never recognized and the raw tag showed up in the message text.

**Fix:** replaced the two ad-hoc `r"<speak>(.*?)</speak>"` patterns with one shared module-level `_SPEAK_BLOCK_RE = re.compile(r"<[^>]*\bspeak\b[^>]*>(.*?)</[^>]*\bspeak\b[^>]*>", re.DOTALL)`, which only requires "speak" to appear as a whole word somewhere inside each pair of angle brackets — tolerating stray characters like the reported `<s speak>` without loosening enough to match unrelated tags (the `\b` word-boundary means e.g. `<speaker>` still would not match). `extract_speak_text` and `parse_message_segments` were both rewritten around `_SPEAK_BLOCK_RE.finditer(text)` instead of `re.findall`/`re.split`+`re.match`, which is also simpler than the previous split-then-rematch approach and behaves identically for well-formed input.
